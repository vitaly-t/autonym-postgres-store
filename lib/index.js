...
function PostgresStoreFactory(connectionString) {
	...
	class PostgresStore {
		...
		findOne(id) {
			...
			return this.one('SELECT * FROM $1^$2^ LIMIT $3', [this.table, clause, 1]);
		}

		create(attributes) {
			let columns = _lodash2.default.keys(attributes).map(_pgPromise2.default.as.name).join();
			let values = _lodash2.default.values(attributes);
		...
		findOneAndUpdate(id, attributes) {
			...
			return this.one('UPDATE $1^ u SET $2^ FROM' + '(SELECT * FROM $1^$3^ LIMIT $4 FOR UPDATE) s ' + 'WHERE s.$5~ = u.$5~ RETURNING u.*', [this.table, sets, clause, 1, 'id']);
		}
		...
		findOneAndDelete(id) {
			...
			return this.one('DELETE FROM $1^ WHERE $2~ IN (' + '  SELECT $2~ FROM $1^$3^ LIMIT $4' + ') RETURNING $5', [this.table, 'ctid', clause, 1, 'id']);
		}
